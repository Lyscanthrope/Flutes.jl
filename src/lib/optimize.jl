...
function optimal(𝒇, 𝒅₋, 𝒅₊, 𝒑₋, 𝒑₊; trace=false)
  ...
  𝒅₁ = [8.8, 8.8, 6.6, 8.8, 9.9, 5.5]
  𝒅₁=𝒅₊*3/4
  # 𝒅₁ = [8.8, 8.8, 8.8, 6.6, 8.8, 9.9, 5.5]
  # simulated annealing (round 1, fast cooldown)
  algo = SAMIN(rt=0.9)
  options = Optim.Options(iterations=Int(1e5), show_trace=trace, show_every=Int(1e4))
  𝒅₂ = Optim.minimizer(optimize(errfn, 𝒅₋, 𝒅₊, 𝒅₁, algo, options))
  # simulated annealing (round 2, slow cooldown)
  algo = SAMIN(rt=0.995)
  options = Optim.Options(iterations=Int(1e6), show_trace=trace, show_every=Int(1e4))
  result = optimize(errfn, 𝒅₋, 𝒅₊, 𝒅₂, algo, options)
